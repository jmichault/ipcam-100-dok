...
struct IMPIVSInterface {
	...
	int (*init)(IMPIVSInterface *inf);								/**< Fonction d'initialisation */
	void (*exit)(IMPIVSInterface *inf);								/**< Fonction de déconnexion */
	int (*preProcessSync)(IMPIVSInterface *inf, IMPFrameInfo *frame);/**< Fonction de prétraitement ， Passé incorrectement dans cette fonction frame Verrou supplémentaire ， Donc pas besoin free frame， valeur de retour: >=0 correct ，<0 :Erreur */
	int (*processAsync)(IMPIVSInterface *inf, IMPFrameInfo *frame);/**< Fonction de traitement , SDK IVS Le module est frame Verrou supplémentaire ， Par conséquent, la fonction doit être frame Utiliser dès que possible après utilisation free_data Déverrouillage des fonctions ; Cette fonction doit être implémentée , Le résultat de l'algorithme est produit par cette fonction ; valeur de retour :0-> Le test réel revient normalement ,1-> La détection de saut de trame revient normalement ,-1-> Erreur */
	int (*getResult)(IMPIVSInterface *inf, void **result);			/**< Obtenir des ressources sur les résultats */
	int (*releaseResult)(IMPIVSInterface *inf, void *result);		/**< Libérer les ressources de résultats */
	int	 (*getParam)(IMPIVSInterface *inf, void *param);			/**< Obtenir les paramètres de l'algorithme */
	int	 (*setParam)(IMPIVSInterface *inf, void *param);			/**< Définir les paramètres de l'algorithme */
	int	 (*flushFrame)(IMPIVSInterface *inf);						/**< Libération par l'extérieur processAsync Tous mis en cache après l'entrée dans l'algorithme frame */
	void *priv;														/**< Variable privée */
};
 ...
 * @retval -1 échec 
 *
 * @remark Le numéro de canal est Chnnum de IVS Le canal de fonction cesse de recevoir des images ， Suspendre l'analyse intelligente 
 *
 * @attention non 
 */
 ...
 * @param[in] ChnNum IVS Numéro de canal correspondant à la fonction 
 *
 * @param[in] result IVS Le résultat de sortie du numéro de canal correspondant à la fonction 
 *
 * @retval 0 Succès 
 * @retval -1 échec 
 *
 * @remark Selon différents IVS Canal lié fonctionnellement , Libérer ses ressources de résultat de sortie .
 *
 * @attention non 
 */
 ...
 * @retval -1 échec 
 *
 * @remark Doit utiliser cette fonction pour libérer la passe à Datacallback de frame paramètre ， Sinon, cela entra je nera certainement une impasse. 
